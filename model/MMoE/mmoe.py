...
class MMoE(Layer):
    ...
    def __init__(self,
        ...
        # Activity parameter, 该参数没有用到， 直接删掉
    ...
    def build(self, input_shape):
        ...
        # 科普下axes参数， 该参数是将int类型的值， 映射为其他数字 如下 将-1映射为输入维度
    ...
    def call(self, inputs, **kwargs):
        ...
        for gate_output in gate_outputs:
            # 将gate_output的维度从B*self.num_experts 改为 B*1*self.num_experts
            expanded_gate_output = K.expand_dims(gate_output, axis=1)
            # repeat_elements沿着第一维， 将元素值进行重复 维度进一步变成 B * U * self.num_experts
            # expert_outputs维度: B * units_num * self.num_experts
            # 所以这里的相乘就是对应元素相乘的了， 输出也是B * units_num * self.num_experts
            weighted_expert_output = expert_outputs * K.repeat_elements(expanded_gate_output, self.units, axis=1)
            # sum之后的维度变成B * units_num 放入元组之中
